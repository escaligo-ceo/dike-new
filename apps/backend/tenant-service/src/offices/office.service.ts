...
export class OfficeService {
  ...
  async createTeamAndInvitations(
    ...
    this.logger.log(`[OfficeService] createAndInvite: ${JSON.stringify(dto)}`);
    // 1. Crea il team
  ...
  async createOffice(loggedUser: LoggedUser, dto: Partial<Office>): Promise<Office> {
    this.logger.log(
      `[OfficeService] createOffice called by userId: ${loggedUser.id} with dto: ${JSON.stringify(
        dto
      )}`
    );
