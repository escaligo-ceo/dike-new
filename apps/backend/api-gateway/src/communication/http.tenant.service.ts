...
export class HttpTenantService extends BaseHttpService {
  ...
  async getTenantsByOwnerId(
    ...
    const userId = ownerId || loggedUser.id;

    const urlSearchParams = new URLSearchParams();
    urlSearchParams.append("ownerId", ownerId ? ownerId : loggedUser.id);
    this.logger.debug(
      `[getTenantsByOwnerId] ownerId: ${userId}, queryParams: ${inspect(
        urlSearchParams.toString()
      )}`
    );

    const response = await this.get(
      `/v1/tenant?${urlSearchParams.toString()}`,
  ...
  async findOrCreateTenantForOwner(
    ...
    this.logger.log(
      `[findOrCreateTenantForOwner] tenantData: ${inspect(tenantData)}`
    );
  ...
  async findOrCreateOfficeOnTenant(
    ...
    this.logger.log(
      `[findOrCreateOfficeOnTenant] tenantId: ${tenantId}, officeData: ${inspect(officeData)}`
    );
  ...
  async findOrCreateMembershipBetweenTenantAndUser(
    ...
    this.logger.log(
      `[findOrCreateMembershipBetweenTenantAndUser] tenantId: ${tenantId}, userId: ${userId}, role: ${role}`
    );
  ...
  async findOrCreateOfficeByTenant(
    ...
    this.logger.log(
      `[findOrCreateOfficeByTenant] tenantId: ${tenantId}, officeData: ${inspect(officeData)}`
    );
